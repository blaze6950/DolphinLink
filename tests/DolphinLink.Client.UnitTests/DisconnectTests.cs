...
/// <summary>
/// Tests that verify <see cref="RpcClient"/> does not hang when the
...
public sealed class DisconnectTests : IAsyncLifetime, IAsyncDisposable
{
    ...
    public async Task InitializeAsync()
    {
        _transport.EnqueueResponse(
            """{"t":0,"i":1,"p":{"n":"dolphin_link_rpc_daemon","v":5,"cmds":["ping","daemon_info","input_listen_start","stream_close"]}}""");
    ...
    [Fact]
    public async Task SendAsync_ThrowsRpcException_AfterTransportEof()
    {
    ...
    [Fact]
    public async Task SendStreamAsync_ThrowsRpcException_AfterTransportEof()
    {
    ...
    [Fact]
    public async Task SendAsync_ThrowsRpcException_AfterDaemonExitEnvelope()
    {
    ...
    public async Task StreamEnumeration_Exits_AfterTransportEof()
    {
        ...
        // Must be a DisconnectedException with the correct reason —

...
/// <see cref="RpcClient"/> instances.  These tests require exclusive
...
///
/// xUnit runs collections sequentially, so both <see cref="DeviceCollection"/>
...
public sealed class HeartbeatTests
{
    ...
    /// <see cref="RpcClient.Disconnected"/> is NOT cancelled after 15 s
    ...
    public async Task HostDispose_DaemonReleasesResources_ReconnectCanAcquireSameResource()
    {
        ...
        var firstClient = new RpcClient(new SerialPortTransport(_portName));
        ...
        await using var secondClient = new RpcClient(new SerialPortTransport(_portName));
        await secondClient.ConnectAsync();

        // If daemon did not release resources this throws RpcException("resource_busy").

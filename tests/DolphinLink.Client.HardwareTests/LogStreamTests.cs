...
/// <see cref="RpcClient"/> instances per test.  These tests require
/// exclusive access to the serial port so they CANNOT share the
/// <see cref="DeviceCollection"/> fixture (which holds the port open for the
...
///
/// Every test opens its own <see cref="RpcClient"/> so it can supply
/// custom <see cref="RpcClientOptions"/> and a capturing sink.
///
...
public sealed class LogStreamTests
{
    ...
    /// that need their own <see cref="RpcClient"/> instance.
    /// </summary>
    ...
    public async Task Diagnostics_PingProducesExpectedEntries()
    {
        ...
        await using var client = new RpcClient(new SerialPortTransport(_portName), diagnostics: sink);
    ...
    public async Task DaemonDiagnostics_ConfigureEnablesDx_ResponseEchoesDxTrue()
    {
        ...
        await using var client = new RpcClient(
            ...
            new RpcClientOptions { DaemonDiagnostics = true });
        await client.ConnectAsync();
    ...
    /// <summary>
    /// When daemon diagnostics are enabled (via <see cref="RpcClientOptions.DaemonDiagnostics"/>),

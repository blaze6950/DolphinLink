...
namespace DolphinLink.Client.HardwareTests.Ui;

/// <summary>
/// Hardware tests for the host-driven UI canvas commands via
/// <see cref="Extensions.UiExtensions.UiScreenAcquireAsync"/> and
/// <see cref="ScreenSession"/>.
///
...
public sealed class UiScreenTests(DeviceFixture fixture)
{
    private RpcClient Client => fixture.Client;

    /// <summary>
    /// <see cref="Extensions.UiExtensions.UiScreenAcquireAsync"/> must succeed and
    /// return a non-null <see cref="ScreenSession"/>.
    ...
    /// <see cref="RpcException"/> with error code <c>resource_busy</c>.
    ...
    /// <summary>
    /// After disposing a <see cref="ScreenSession"/>, the caller must be
    ...
    /// <summary>
    /// Calling <see cref="ScreenSession.DisposeAsync"/> more than once
    /// must not throw.
    /// Validates: the idempotency guard (<c>Interlocked.Exchange</c>) in
    /// <see cref="ScreenSession.DisposeAsync"/> prevents a double

...
/// </summary>
internal sealed class FlipperNativeRpcClient : IAsyncDisposable
{
    ...
    private async Task<Main> SendAsync(Main request, CancellationToken ct)
    {
        ...
        {
            throw new FlipperNativeRpcException(response.CommandStatus,
    ...
    /// </summary>
    internal async Task<Proto.File?> StorageStatAsync(string path, CancellationToken ct = default)
    {
        var req = new Main
        {
            CommandId          = NextId(),
        ...
        catch (FlipperNativeRpcException ex) when (ex.Status == CommandStatus.ErrorStorageNotExist)
        {
            return null;
        }
    }
    ...
    internal async Task StorageMkdirAsync(string path, CancellationToken ct = default)
    {
        ...
        catch (FlipperNativeRpcException ex) when (ex.Status == CommandStatus.ErrorStorageExist)
        {
        ...
        CancellationToken ct = default)
    {
        ...
        while (written < total || total == 0)
        {
            ...
            var req = new Main
            {
                ...
                {
                    Path = path,
                    File = new Proto.File { Data = chunk },
                },
            };
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        // pattern fixed in FlipperRpcClient / PacketSerializationTransport.
...
/// </summary>
public sealed class FlipperNativeRpcException : Exception
{
    /// <summary>The error status returned by the device.</summary>
    public CommandStatus Status { get; }

    internal FlipperNativeRpcException(CommandStatus status, string message)
        : base(message)
    {
        Status = status;
    }

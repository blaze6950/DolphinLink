...
///       ↑
///   RpcClient                    (this class — RPC logic only)
/// </code>
///
/// Both decorator layers are optional and can be disabled via
/// <see cref="RpcClientOptions.DisablePacketSerialization"/> and
/// <see cref="RpcClientOptions.DisableHeartbeat"/>.  This is useful in
...
/// <see cref="RpcClient(ITransport,RpcClientOptions,IRpcDiagnostics)"/>
...
public sealed class RpcClient : IAsyncDisposable
{
    ...
    /// rethrow the exact same <see cref="DisconnectedException"/> (with the
    ...
    /// </summary>
    private volatile DisconnectedException? _faultException;
    ...
    /// The client takes ownership: it will call <see cref="ITransport.OpenAsync"/>
    ...
    public RpcClient(
        ...
        RpcClientOptions options = default,
        IRpcDiagnostics? diagnostics = null)
    {
        // Build the transport chain: raw → [serializer] → [heartbeat]
        // Both decorator layers are optional; see RpcClientOptions.
        ITransport current = transport;
    ...
    /// equals <c>"dolphin_link_rpc_daemon"</c>, and checks that the daemon's protocol
    /// version is at least <paramref name="minProtocolVersion"/>.
    ///
    /// If the daemon supports the <c>configure</c> command (protocol version &gt;= 4),
    /// the client's <see cref="RpcClientOptions"/> are propagated to the daemon
    ...
    /// Throws <see cref="RpcException"/> if the daemon reports a lower version.
    /// </param>
    ...
    {
        const string ExpectedName = "dolphin_link_rpc_daemon";
        ...
        if (info.Name != ExpectedName)
        {
            throw new RpcException(
                ...
                $"got '{info.Name ?? "(null)"}'. " +
                "Ensure the DolphinLink RPC daemon FAP is running on the device.");
        }
        ...
        if (info.Version < minProtocolVersion)
        {
            throw new RpcException(
                ...
                "Please update the DolphinLink RPC daemon FAP on the device.");
        }
        ...
        else if (_options.DisableHeartbeat)
        {
            ...
            throw new RpcException(
                ...
                "Update the DolphinLink RPC daemon FAP, or set DisableHeartbeat=false.");
        }
        ...
        where TResponse : struct, IRpcCommandResponse
    {
        ...
        if (Volatile.Read(ref _faulted) == 1)
        {
            throw _faultException
                ?? new DisconnectedException(DisconnectReason.ConnectionLost, "Connection lost.");
        }
    ...
    private async Task ReaderLoopAsync(CancellationToken ct)
    {
        try
        {
            await foreach (var line in _transport.ReceiveAsync(ct).ConfigureAwait(false))
            {
                ...
                {
                    FaultAll(new DisconnectedException(
    ...
    private void FaultAll(DisconnectedException ex)
    {
        ...
        // so that stream consumers see DisconnectedException rather than
        ...
        // will throw DisconnectedException rather than OperationCanceledException.
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        FaultAll(new DisconnectedException(DisconnectReason.ClientDisposed, "Client disposed."));

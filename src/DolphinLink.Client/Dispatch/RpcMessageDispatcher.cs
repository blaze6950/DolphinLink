...
///
/// Injected dependencies (all <see cref="RpcClient"/> internals):
/// <list type="bullet">
...
/// The <see cref="RpcMessageType.Disconnect"/> case is intentionally NOT handled here.
    /// It is handled directly in the reader loop of <see cref="RpcClient"/> so that
...
internal sealed class RpcMessageDispatcher
{
    ...
    /// <see cref="RpcClient"/> can use the same instance as its own
    ...
    private void DispatchResponse(RpcEnvelope envelope, string rawLine, long receivedTimestamp)
    {
        ...
        {
            pending.Fail(new RpcException(requestId, errorCode));
        }
        else
        {

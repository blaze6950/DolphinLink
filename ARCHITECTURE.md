...
- `NegotiateAsync` sends a `configure` command with `heartbeatMs = 3,600,000` (1 h) and
  ...
  either. (Requires the daemon to advertise the configure command; throws `RpcException` otherwise.)
...
`RpcJsonNormalizer` (namespace `DolphinLink.Client`) transforms compact wire-format JSON into
...
- `RpcJsonNormalizer.cs` — hand-written; contains the `WriteNormalized()` and related private
  helper methods.

All lookup tables are switch expressions for zero-allocation, JIT-optimized dispatch. Output is
produced via `ArrayBufferWriter<byte>` + `Utf8JsonWriter`, then decoded with `Encoding.UTF8.GetString()`.
